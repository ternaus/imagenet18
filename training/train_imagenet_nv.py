...
def get_parser():
    ...
    parser.add_argument(
        "--print-freq", "-p", default=50, type=int, metavar="N", help="log/print every this many steps (default: 50)"
    )
    ...
    parser.add_argument(
        "--skip-auto-shutdown", action="store_true", help="Shutdown instance at the end of training or failure"
    )
    parser.add_argument(
        "--auto-shutdown-success-delay-mins",
        default=10,
        type=int,
        help="how long to wait until shutting down on success",
    )
    parser.add_argument(
        "--auto-shutdown-failure-delay-mins",
        default=60,
        type=int,
...
IS_CHIEF = RANK == "0"
print(
    f"*** Debug: {os.uname()[1]} RANK={RANK} local_rank_arg={args.local_rank} LOCAL_RANK={LOCAL_RANK}, "
    f"OMPI_COMM_WORLD_LOCAL_RANK={OMPI_COMM_WORLD_LOCAL_RANK}, {' '.join(sys.argv)}"
)
...
def main():
    ...
    optim_params = (
        experimental_utils.bnwd_optim_params(model, model_params, master_params) if args.no_bn_wd else master_params
    )

    # define loss function (criterion) and optimizer
    criterion = nn.CrossEntropyLoss().cuda()
    optimizer = torch.optim.SGD(
        optim_params, 0, momentum=args.momentum, weight_decay=args.weight_decay
...
def train(trn_loader, model, criterion, optimizer, scheduler, epoch):
    ...
    for i, (input, target) in enumerate(trn_loader):
        ...
        if args.distributed:
            # Must keep track of global batch size, since not all machines are
            # guaranteed equal batches at the end of an epoch
